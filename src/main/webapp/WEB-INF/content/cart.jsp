	...
	<div id="page-content" class="single-page">
		<div class="container">
			<div class="row">
				<div class="col-lg-12">
					<ul class="breadcrumb">
						<li><a href="showMain?username=${user.username}">主页</a></li>
						<li>购物车</li>
					</ul>
				</div>
			</div>
 			<div class="row"><c:forEach items="${requestScope.cart_list}" var="cart">
				<div class="product well">
					...
					<div class="col-md-3">
						<div class="image">
							<img src="images/${cart.goods.image}" />
						</div>
					</div>
					<div class="col-md-8">
						<div class="caption">
							<div class="name"><h3> ${cart.goods.name}</h3></div>
							<div class="price"> ${cart.goods.price}</div>
							<form action="updateCart" method="get">							
							<label>数量： </label> <input class="form-inline quantity" type="text" name="number" value="${cart.number}"/>
							<input type="hidden" name="id" value="${cart.id}"/>
							<input type="hidden" name="user.username" value="${user.username}"/>
							<input type="submit" value="Update" class="btn btn-2 "/>
							</form>
							<hr>
							<a href="deleteCart?goods.id=${cart.goods.id}&user.id=${cart.user.id}&username=${user.username}" class="btn btn-default pull-right">删除</a>
						</div>
					</div>
					<div class="clear"></div>
				</div>	
			...
			<div class="row">
				...
				<div class="col-md-3 ">
					<form action="addOrder" method="get"><c:forEach items="${requestScope.cart_list}" var="cart">
						<input type="hidden" name="user.id" value="${user.id}"/>
						<input type="hidden" name="goods.id" value="${cart.goods.id}"/>
						<input type="hidden" name="number" value="${cart.number}"/>
						</c:forEach>
						<center>
						<input type="hidden" name="username" value="${user.username}"/>
						<input type="submit"  value="提交订单" class="btn btn-1"></center>
					</form>
				</div>
			</div>
		</div>
